...
export const ApiClient = {
	...
	alias<const Alias extends Aliases<Api>>(
		...
		const body: ReadonlyDeep<UndefinedIfNever<AliasBody<Alias>>> | undefined =
			isMutation(
				// biome-ignore lint/style/noNonNullAssertion: alias is typed, so assertion is safe
		...
		if (
			...
			res = (this.client[alias] as unknown as AliasFunctionGet<Alias>)(
				// biome-ignore lint/style/noNonNullAssertion: options is null iff function takes in null
				options!,
			);
		} else {
			res = (this.client[alias] as unknown as AliasFunctionMut<Alias>)(
				// biome-ignore lint/style/noNonNullAssertion: body cannot be null if mutation
				body!,
				// biome-ignore lint/style/noNonNullAssertion: options is null iff function takes in null
				options!,
			);
