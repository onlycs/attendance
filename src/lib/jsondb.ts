...
type SchemaOf<Table extends DbTable, Name extends Names<Table>> = ColumnOf<
	...
	Name
>["schema"];
type TypeOf<Table extends DbTable, Name extends Names<Table>> = z.TypeOf<
	SchemaOf<Table, Name>
>;
...
export class JsonDb<Table extends DbTable> {
	...
	constructor(
		...
		for (const row of toParse) {
			...
			for (const [key, value] of Object.entries(row)) {
				...
				building[key as keyof DataRow<Table>] =
					parsed as DataRow<Table>[keyof DataRow<Table>]; // idk
			}
		...
		return this.data
			...
			.filter(([data, _]) =>
				Object.entries(data).every(
					([k, v]) => preds[k as Names<Table>]?.(v) ?? true,
				),
	...
	get(filter: DataFilter<Table>) {
		const preds = makePredicates(filter);
		const matching = this.data.filter((data) =>
			Object.entries(data).every(
				([k, v]) => preds[k as Names<Table>]?.(v) ?? true,
			),
		);
	...
	update(
		...
		for (const [data, i] of this.data.map((a, i) => [a, i] as const)) {
			if (
				!Object.entries(data).every(
					([k, v]) => preds[k as Names<Table>]?.(v) ?? true,
				)
			...
			this.data[i] = updatefn(data); // update the data
		}
	}
